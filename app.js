...
const updatePage = async () => {
  ...
  var fruits_list = fruitsArray.filter((fruit) => {
    return fruit.nutritions.sugar > 15
  })
  
  const existingElement = document.getElementById('fruits_list')
  ...
  for (let i = 0; i < fruits_list.length; i++){
    ...
    newElementText.innerHTML = fruits_list[i].name + " belonging to Family:" + fruits_list[i].family + " ";
    // TODO: Append your new element to the page
    existingElement.append(newElementList)
    newElementList.append(newElementText)
  }
}
